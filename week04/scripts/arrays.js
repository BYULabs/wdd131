...
document.getElementById("subjects").innerHTML = subjects.map(subject => `<li>${subject}</li>`).join("");
...
const countriesStartingWithU = countries.filter(country => country.startsWith("U"));
document.getElementById("u").innerHTML = countriesStartingWithU.map(country => `<li>${country}</li>`).join("");
...
const totalFruitCost = fruits.reduce((total, fruit) => total + fruit.price, 0);
document.getElementById("total").innerHTML = `$${totalFruitCost.toFixed(2)}`;
...
document.getElementById("product").textContent = expensiveProduct ? expensiveProduct.name : "No product found";

// *************************************************************************************
// 6️⃣ determine if Canada is contained within the array countries (see line 18)
const canadaIndex = countries.indexOf("Canada");
document.getElementById("output").textContent = canadaIndex !== -1 ? `Canada is at index ${canadaIndex}` : "Canada is not in the array";
